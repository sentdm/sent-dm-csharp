...
///
/// <para>Updates an existing campaign under the brand of the specified profile. Cannot
/// update campaigns that have already been submitted to TCR.</para>
///
...
public sealed record class CampaignUpdateParamsCampaign : JsonModel
{
    ...
    /// <summary>
    /// Campaign type (e.g., "KYC", "App").              Still required of a caller,
    /// and consulted by nothing. It named the signup path that produced the campaign,
    /// was written to a column no query filters on, no TCR payload carries and no
    /// fee or status decision reads, and is now defaulted at the entity instead.
    /// It stays required so that a request which was valid before is still valid
